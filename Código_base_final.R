...
# Bases do IDEB
ideb_munic <- read.csv("ideb_municipios_anosfinais2005_2017.csv",
                       na = "-")
ideb_uf <- read.csv("ideb_uf_regioes_anosfinais2005_2017.csv",
                    na = "-")
...
# Função que busca a nota genérica do estado caso falte do município
resposta_geral <- function(x){
  x = x %>% as.matrix()
  resp = ideb_munic %>% filter(Rede == x[4], Cod_Municipio_Completo == x[2]) %>% 
    select(Ideb2017) %>% as.numeric()
  if (resp %>% is.na() == T){
    resp = ideb_munic %>% filter(Rede == x[4], Cod_Municipio_Completo == x[2]) %>% 
      select(ProjecaoIdeb2017) %>% as.numeric()
  }
  if (resp %>% is.na() == T){
    resp = ideb_uf %>% filter(Rede == "Pública", UF_REG == x[1]) %>% 
      select(Ideb2017) %>% as.numeric()
  }
  return(resp)
}
...
# Função que categoriza a resposta para 1 se maior que a meta e 0 se não
...
# Base de escola
escolas <- read.csv("escolas20170101.csv", encoding = "UTF-8")
...
escolas$in_patio <- pmax(escolas$in_patio_coberto,
                         ...
                         na.rm = T)
escolas$in_equip <- apply(escolas %>% select(in_equip_copiadora,
...
escolas$num_equip <- apply(escolas %>% select(num_equip_copiadora,
...
filtro = escolas %>% filter(rede_publica_desc == "pública") %>% 
...
# Juntando as bases
base_aux = merge(ideb, filtro,
                 by.x = "Cod_Municipio_Completo", by.y = "cod_municipio")
munic = read_excel("Base_Munic_Resumo.xlsx")
base_ideb = merge(aux, munic,
                  by.x = "Cod_Municipio_Completo", by.y = "CodMun")

# saveRDS(base, "Base_Ideb.rds")

municipios = base %>% select(Cod_Municipio_Completo, Nome_Municipio)

# Caminho para as bases
path <- "C:/Users/Carol/Desktop/BCG Gamma Challenge/docencia/"

# Colunas a serem usadas
colunas <- c("cod_municipio", "tp_cor_raca", "tem_ensino_superior", "etapa_agg_fund_ai",
             "etapa_agg_fund_af", "tp_etapa_ensino",
             "num_disciplinas_ensina", "num_formacoes", "num_disc_form_adequada",
             "ensina_disc_lingua_portuguesa", "ensina_disc_lingua_estrangeira", "ensina_disc_artes",
             "ensina_disc_educacao_fisica", "ensina_disc_matematica", "ensina_disc_ciencias",
             "ensina_disc_quimica", "ensina_disc_fisica", "ensina_disc_biologia",
             "ensina_disc_estudos_sociais", "ensina_disc_historia", "ensina_disc_geografia",
             "ensina_disc_sociologia", "ensina_disc_filosofia", "ensina_disc_ensino_religioso",
             ...
             "form_adequada_lingua_estrangeira", "form_adequada_artes", "form_adequada_educacao_fisica",
             "form_adequada_matematica", "form_adequada_ciencias", "form_adequada_quimica",
...
# Bases
bases <- dir(path, full.names = T)

# Leitura e preparação das bases
# docencia <- readRDS(paste0(path, "/docencias012.RDS"))

base = readRDS(bases[1]) %>%
...
docencias <- docencia %>%
  ...
  right_join(
    municipios %>% rename(cod_municipio = Cod_Municipio_Completo),
    by = 'cod_municipio')


# base_ideb = readRDS('Base_Ideb.rds') 


# Criação base final
base_final = left_join(base_ideb %>%  
                         rename(cod_municipio = Cod_Municipio_Completo),
                       docencias,
                       by = 'cod_municipio')
